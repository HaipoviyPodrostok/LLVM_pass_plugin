...
private:
  Module &M;
  std::map<Value *, uint64_t>
      IDmap; // FIXME[flops]: You don't need ordering there, so you can pick
             // another container for those purposes: std::unordered_map /
             // llvm::DenseMap e.t.c. std:: API is better, because LLVM API is
             // unstable and it can change from version to version

  void indexUnits();
  void makeDotFile();
  void addInstrumentation();
  void writeDotHeader(std::ostream &OS) const;
  void argsDumping();
};
...
void DefUseGraph::writeDotHeader(raw_fd_ostream &OS) const {
...
void DefUseGraph::makeDotFile() {
  ...
  for (Function &F : M) {
    ...
    if (F.arg_size() > 0) { // FIXME[flops]: Separate method for args dumping
      ...
      for (Argument &A : F.args()) {
        ...
        File << "    " << IDmap[&A] << " [label=\""
             << llvm::DOT::EscapeString(label) << nodeStyle;
      }
      File << "  }\n";
    }
  ...
  for (GlobalVariable &GV : M.globals()) {
    ...
    File << "  " << IDmap[&GV] << " [label=\"" << llvm::DOT::EscapeString(label)
         << nodeStyle;
  }
...
void DefUseGraph::addInstrumentation() {
  ...
  FunctionCallee LogFunc = M.getOrInsertFunction(
      "__log_value", Type::getVoidTy(Context), Int64Type, Int64Type);
...
PreservedAnalyses DefUseGraphPass::run(Module &M, ModuleAnalysisManager &) {
  ...
  return PreservedAnalyses::all(); // There are cases when you instrument
                                   // nothing, so it should return
                                   // `PreservedAnalyses::none();`, otherwise
